...
def command_line_parser():
    ...
    model_name = args['--name'] if args['--name'] else '0'
    ...
    if args['selfplay']:
        ...
        elif args['keras_dqn_train']:
            runner.dqn_train_keras_rl(model_name)

        elif args['keras_dqn_play']:
            runner.dqn_play_keras_rl(model_name)

        elif args['tune']:
            runner.tune_params()

        elif args['pytorch_dqn_train']:
            runner.dqn_train_pytorch(model_name)
        
        elif args['pytorch_dqn_play']:
            runner.dqn_play_pytorch(model_name)

        elif args['keras_ddqn_train']:
            runner.ddqn_train_keras_rl(model_name)

        elif args['keras_ddqn_play']:
            runner.ddqn_play_keras_rl(model_name)
    else:
...
class SelfPlay:
    ...
    def dqn_train_keras_rl(self, model_name):
        ...
        env.add_player(RandomPlayer())
        env.add_player(RandomPlayer())
        env.add_player(PlayerShell(name='keras-rl-dqn', stack_size=self.stack))
        env.reset()
        ...
        '''
        env.add_player(PlayerShell(name='keras-rl-dqn', stack_size=self.stack))  # shell is used for callback to keras rl
        env.add_player(PlayerShell(name='keras-rl-dqn1', stack_size=self.stack))
        env.add_player(PlayerShell(name='keras-rl-dqn2', stack_size=self.stack))
        env.add_player(PlayerShell(name='keras-rl-dqn3', stack_size=self.stack))
        env.add_player(PlayerShell(name='keras-rl-dqn4', stack_size=self.stack))
        env.add_player(PlayerShell(name='keras-rl-dqn5', stack_size=self.stack))

        
        players = []
        for i in range(6):
            dqn_player = DQNPlayer()
            dqn_player.initiate_agent(env)
            players.append(dqn_player)

        for player in players:
            player.train(env_name=model_name)'''
    ...
    def ddqn_train_keras_rl(self, model_name):
        """Implementation of kreras-rl deep q learing."""
        from agents.agent_consider_equity import Player as EquityPlayer
        from agents.agent_keras_rl_ddqn import Player as DDQNPlayer
        from agents.agent_random import Player as RandomPlayer
        env_name = 'neuron_poker-v0'
        env = gym.make(env_name, initial_stacks=self.stack, funds_plot=self.funds_plot, render=self.render,
                       use_cpp_montecarlo=self.use_cpp_montecarlo, epochs_max=self.epochs_max, to_log=self.to_log)
        ...
        env.add_player(RandomPlayer())
        env.add_player(RandomPlayer())
        env.add_player(PlayerShell(name='keras-rl-ddqn', stack_size=self.stack))  # shell is used for callback to keras rl
    ...
    def ddqn_play_keras_rl(self, model_name):
        """Create 6 players, one of them a trained DQN"""
        from agents.agent_consider_equity import Player as EquityPlayer
        from agents.agent_keras_rl_ddqn import Player as DDQNPlayer
        from agents.agent_random import Player as RandomPlayer
        ...
        dqn = DDQNPlayer(load_model=model_name, env=self.env)
        dqn.play(nb_episodes=self.num_episodes, render=self.render)
